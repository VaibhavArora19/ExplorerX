...
import { deployerAbi, contractAddress, connextDomains, rpcUrls } from "@/constants";
import { useRouter } from "next/router";
const Backdrop = ({ onClose }) => {
  return <div onClick={onClose} className="top-0 left-0 fixed bg-black/20 backdrop-blur-md h-screen w-screen"></div>;
};

const DeployModal = ({ onClose, bytecode, formData, setFormData, initializable, initializableData, abi }) => {
  ...
  const deployContractHandler = async () => {
    try {
      ...
      let fees = [];
      let tx;
      if (formData.currentDeployChain.chainName in connextDomains && formData.multichains.length > 0) {
        ...
        tx = await contract.xDeployer(contractAddress, domains, saltbytes, bytecode, fees, initializable, initializableData, totalFee, {
          value: totalFee,
        });
        console.log(tx, "tx");
        ...
        for (let i = 0; i < selectedChains.length; i++) {
          if (!(selectedChains[i].chainName in connextDomains)) {
            console.log(selectedChains[i].chainName, "selectedChains[i].chainName");
            const provider = new ethers.providers.JsonRpcProvider(rpcUrls[selectedChains[i].chainName]);
            const wallet = new ethers.Wallet(process.env.NEXT_PUBLIC_PRIVATE_KEY, provider);
            const account = wallet.connect(provider);
            let tx = await contract.connect(account).deployContract(saltbytes, bytecode, initializable, initializableData, {
              gasPrice: 1000000000,
            });
            console.log(tx, "tx");
          }
        ...
        const selectedChains = formData.multichains;
        tx = await contract.deployContract(saltbytes, bytecode, initializable, initializableData, { gasPrice: 1000000000 });
        console.log(tx, "tx");
        for (let i = 0; i < selectedChains.length; i++) {
          const provider = new ethers.providers.JsonRpcProvider(rpcUrls[selectedChains[i].chainName]);
          const wallet = new ethers.Wallet(process.env.NEXT_PUBLIC_PRIVATE_KEY, provider);
          const account = wallet.connect(provider);
          let tx = await contract.connect(account).deployContract(saltbytes, bytecode, initializable, initializableData, { gasPrice: 1000000000 });
          console.log(tx, "tx");
        }
      }
      await tx.wait();
      ...
      <div className="w-[550px] bg-[#111111] p-10 rounded-2xl absolute top-[50%] left-[50%] shadow-md -translate-x-[50%] -translate-y-[50%] z-10 rounded-b-2xl  overflow-hidden border border-gray-800">
        {startDeploying ? (
          <div className="flex flex-col justify-center items-center gap-4">
            ...
            <p className="text-green-300 text-sm">{deploymentSuccess ? "Deployment Success" : "Deploying Contract..."}</p>

            {deploymentSuccess && (
              <button
                onClick={() => {
                  router.push(`/address/${computedAddress}/?chain=${formData.currentDeployChain.chainName.toLowerCase().split(" ")[0]}`);
                }}
          ...
          <div>
            <div className="flex flex-col justify-center items-center gap-4">
              ...
              <p className="text-green-300 text-sm">
                {generatingAddress ? (computedAddress ? "Generated Address" : "Generating Address...") : "Successfully Compiled!"}
              </p>
            </div>

            <>
              <p className="text-xs text-gray-400 mt-6 mb-1">Enter the salt value</p>
              <input
              ...
              {computedAddress !== "" && (
                <div>
                  <p className="text-xs text-gray-400 mt-6 mb-1">Generated Address</p>
              ...
              <p className="text-xs mt-1 text-gray-400 text-center">Generate an address for your compiled contract</p>
            </>
            {computedAddress !== "" && (
              <div>
                <p className="text-xs text-gray-400 mt-4 mb-1">
                  If you like the address generated then you can go for deployment otherwise you can alter the salt and generate new address
                </p>
                <button
